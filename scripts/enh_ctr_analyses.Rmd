...
total_catch <- read_csv(here("processed", "harvest/totcatch/total_catch.csv"))
CN_all_catch <- read_csv(here("processed", "harvest/totcatch/CN_ALL_catch.csv"))
CO_all_catch <- read_csv(here("processed", "harvest/totcatch/CO_ALL_catch.csv"))

# Chum dat from Lynch et al. 2020 (raw data processed in the 'chum_wrangle.Rmd' script)
CM_dat <- read_csv(here("processed", "harvest/CU/chum_all.csv"))
...
#ggsave("../figs/harvest/totcatch/AllBC_CN_totcatch_bar.png", width = 20, height = 22, units = "cm")
...
ggplot(subset(total_catch, species == "Coho"), aes(x = year, y = catch, fill = fishery)) +
  ...
  labs(y = "Total Catch", fill = "Fishery") +
  scale_y_continuous(labels = comma)
#ggsave("../figs/harvest/totcatch/AllBC_CO_totcatch_bar.png", width = 20, height = 22, units = "cm")
...
ggplot(subset(total_catch, species == "Chum"), aes(x = year, y = catch, fill = fishery)) +
  ...
  labs(y = "Total Catch", fill = "Fishery") +
  scale_y_continuous(labels = comma)
#ggsave("../figs/harvest/totcatch/AllBC_CM_totcatch_bar.png", width = 20, height = 22, units = "cm")
...
#write_csv(CN_totcatch, "../processed/harvest/totcatch/CN_totcatch.csv")
...
CO_totcatch <- total_catch %>%
  ...
  pivot_wider(names_from = region_fishery, values_from = catch) %>%
  select(year, NCST_Net, NCST_Troll, NCST_Rec, CCST_Net, CCST_Troll, CCST_Rec, WCVI_Net, WCVI_Troll, WCVI_Rec, ISC_Net, ISC_Troll, ISC_Rec)
#write_csv(CO_totcatch, "../processed/harvest/totcatch/CO_totcatch.csv")

CM_totcatch <- all_catch %>%
  ungroup() %>%
  filter(species == "Chum") %>%
  select(year, area, catch, fishery) %>%
  mutate(region = case_when(area %in% c(1:5) ~ "NCST",
  ...
  pivot_wider(names_from = region_fishery, values_from = catch) %>%
  select(year, NCST_Net, NCST_Troll, NCST_Rec, CCST_Net, CCST_Troll, CCST_Rec, WCVI_Net, WCVI_Troll, WCVI_Rec, ISC_Net, ISC_Troll, ISC_Rec)
#write_csv(CM_totcatch, "../processed/harvest/totcatch/CM_totcatch.csv")


# To get annual total and median catch by region
CN_hatch_sum <- CN_hatch %>%
  mutate(tperiod = case_when(year < 1996 ~ "Pre-96",
                             year > 1995 ~ "Post-95")) %>%
  group_by(region, year, tperiod) %>%
  summarise(totcatch = sum(CWT_catch)) %>%
...
# To get annual total and median catch by region
CO_hatch_sum <- CO_hatch %>%
  ungroup() %>%
  mutate(tperiod = case_when(year < 1996 ~ "Pre-96",
                             year > 1995 ~ "Post-95")) %>%
  group_by(region, year, tperiod) %>%
  summarise(totcatch = sum(CWT_catch, na.rm = T)) %>%
  group_by(region, tperiod) %>%
  summarise(medcatch = median(totcatch))
...
# Calculate extended hatchery catch and enhanced contributions on a regional scale (** NOTE: Total catch for NCST and CCST is based on commercial catch data only and thus enhanced contribution calcs are not accurate. Fill these 2 columns with NA for report tables)
...
regional_enhctr_CN <- Ext_catch_CN %>%
  ...
  arrange(region)
#write_csv(regional_enhctr_CN, "../processed/harvest/totcatch/CN_enhctr.csv")

# Calculate the overall mean enhanced contribution per region
mean_enhctr <- regional_enhctr_CN %>%
  group_by(region) %>%
...
regional_enhctr_CO <- Ext_catch_CO %>%
  ...
  mutate(region = factor(region, levels = c("NCST", "CCST", "WCVI", "ISC"))) %>%
  arrange(region) 
#write_csv(SBC_regional_enhctr_CO, "../processed/harvest/totcatch/CO_enhctr.csv")

# Calculate the overall mean enhanced contribution per region
mean_enhctr_CO <- SBC_regional_enhctr_CO %>%
  group_by(region) %>%
  summarise(mean_enhctr = mean(exp_prop, na.rm = T), sd = sd(exp_prop, na.rm = T))
...
#ggsave("../figs/harvest/CN/total_hatch_catch.png", width = 18, height = 13, units = "cm")
...
ggplot(hatch_catch_CO, aes(x = year, y = catch)) +
  ...
  scale_fill_manual(values = cols) +
  labs(y = "Total Annual Catch", fill = "Hatchery Data Type")
#ggsave("../figs/harvest/CO/total_hatch_catch.png", width = 18, height = 13, units = "cm")

```

The following code generates Figures 12-13 in the Harvest Report showing enhanced contributions to catch in WCVI and ISC for CN and CO
...
SBC_regional_enhctr_CN <- Ext_catch_CN %>%
  select(1:2, 5:7) %>%
  distinct() %>%
  filter(region %in% c("ISC", "WCVI"), year > 1980) %>%
  group_by(year, region) %>%
...
totcatch_SBC_CN <- ggplot(subset(regional_enhctr_CN, region %in% c("WCVI", "ISC")), aes(x = year, y = `Total Catch`)) +
...
#ggsave("../figs/harvest/CN/SBC_totcatch_hatchprops.png", width = 18, height = 14, units = "cm")
...
#ggsave("../figs/harvest/CO/SBC_totcatch_hatchprops.png", width = 18, height = 14, units = "cm")
...
ggsave("../figs/harvest/totcatch/SBC_enhctr_byregion_CN.png", width = 20, height = 12, units = "cm")
...
#ggsave("../figs/harvest/CN/WCVI_totcatch_hatchprops.png", width = 18, height = 12, units = "cm")
...
#ggsave("../figs/harvest/CN/ISC_totcatch_hatchprops.png", width = 18, height = 12, units = "cm")
...
SBC_fishery_enhctr_CO <- SBC_catch_CO %>%
  filter(region %in% c("ISC", "WCVI"), year > 1980, fishery != "Other") %>%
  mutate(fishery = factor(fishery, levels = c("Net", "Troll", "Rec"))) %>%
  group_by(year, region, type, fishery) %>%
  mutate(`CWT Catch` = sum(CWT_catch, na.rm = T), `Total Hatchery Catch` = `CWT Catch`/CWT_rate, `Total Catch` = sum(tot_catch, na.rm = T)) %>%
...
SBC_fishery_props_CO <- SBC_catch_CO %>%
  filter(region %in% c("ISC", "WCVI"), year > 1980, fishery != "Other") %>%
  mutate(fishery = factor(fishery, levels = c("Net", "Troll", "Rec"))) %>%
  group_by(year, region, type, fishery) %>%
  mutate(`CWT Catch` = sum(CWT_catch, na.rm = T), `BC Hatchery` = `CWT Catch`/CWT_rate, `Total Catch` = sum(tot_catch, na.rm = T),Other = `Total Catch` - `BC Hatchery`) %>%
...
ggsave("../figs/harvest/CO/WCVI_totcatch_hatchprops.png", width = 18, height = 12, units = "cm")
...
#ggsave("../figs/harvest/CO/ISC_totcatch_hatchprops.png", width = 18, height = 12, units = "cm")
...
ggplot(chum_tab1, aes(x = recY)) +
  ...
  scale_fill_manual(values = rev(cols)) +
  labs(y = "Total Annual Catch", fill = "Hatchery Data Type")
#ggsave("../figs/harvest/CU/total_hatch_catch.png", width = 18, height = 13, units = "cm")
...
#ggsave("../figs/harvest/CU/NBC_totcatch_hatchprops.png", width = 18, height = 14, units = "cm")
...
#ggsave("../figs/harvest/CU/SBC_totcatch_hatchprops.png", width = 18, height = 14, units = "cm")
...
CTC_MDT <- read_csv(here("raw", "Chinook_All Mort Tables Long.csv"))
