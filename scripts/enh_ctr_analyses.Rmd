...
regional_enhctr_CO <- Ext_catch_CO %>%
  #filter(year > 1980) %>% # Need to filter to only look at 1981 onwards for enh contribution calcs since only commercial data prior to 1981, but show all data for hatchery plots in next code chunk
  group_by(year, region) %>%
  mutate(`Exp CWT` = sum(CWT_catch, na.rm = T), `Total Hatchery` = `Exp CWT`/CWT_rate, `Total Catch` = sum(tot_catch, na.rm = T)) %>%
  mutate(`Total Hatchery` = case_when(`Total Hatchery` == Inf ~ `Exp CWT`,
                                      TRUE ~ `Total Hatchery`)) %>%
  mutate(`Enhanced Contribution` = `Total Hatchery`/`Total Catch`) %>%
  select(1:2, 8:11) %>%
  distinct() %>%
  mutate(region = factor(region, levels = c("NCST", "CCST", "WCVI", "ISC"))) %>%
  arrange(region) 
...
# Calculate the overall mean enhanced contribution for WCVI and ISC
mean_enhctr_CO_SBC <- regional_enhctr_CO %>%
  filter(region %in% c("WCVI", "ISC")) %>%
  group_by(region) %>%
  summarise(mean_enhctr = mean(`Enhanced Contribution`, na.rm = T), sd = sd(`Enhanced Contribution`, na.rm = T))

# Calculate the mean enhanced contribution for NCST and CCST (has to be from 2013 onwards as these are the only years for which we have rec and commercial catch)
mean_enhctr_CO_NBC <- regional_enhctr_CO %>%
  filter(region %in% c("NCST", "CCST"), year > 2012) %>%
  group_by(region) %>%
  summarise(mean_enhctr = mean(`Enhanced Contribution`, na.rm = T), sd = sd(`Enhanced Contribution`, na.rm = T))
...
## --- ISC ---
# Plot of total CN catch for ISC:
totcatch_ISC_CN <- ggplot(subset(fishery_enhctr_CN, source == "Total Catch" & region == "ISC"), aes(x = year, y = catch)) +
  geom_bar(stat = "identity", position = "dodge", colour = "white", size = 0.3) +
  facet_wrap(~fishery, scales = "free_y") +
  theme_bw() +
  theme(axis.text.x = element_blank(),
...
# Plot the proportions of hatchery and non-hatchery catch
hatch_props_ISC_CN <- ggplot(subset(fishery_props_CN, region == "ISC"), aes(x = year, y = catch, fill = source)) +
  geom_bar(stat = "identity", position = "fill", colour = "white", size = 0.3) +
  facet_wrap(~fishery, scales = "free_y") +
  theme_bw() +
  theme(legend.position = "bottom",
...
CN_areaplot
ggsave("../figs/distribution/BCwide_CN_catchbyfishery_areaplot.png", width = 16, height = 12, units = "cm")
...
##---- CN ---- 
# Group some of the fisheries together into broader categories
CN_catchdist_byarea <- CN_CatchbyFishery %>%
  mutate(fishery = case_when(fishery %in% c("BC Unknown Freshwater", "Freshwater S", "Kamloops Lake") ~ "FW",
                             fishery %in% c("ADFG", "NMFS AK", "Alaskan Net (AK Catch, sampled by Canada)", "Alaskan Troll (Historical)") ~ "AK",
                             fishery %in% c("PFMA8 Terminal N", "Central N") ~ "Central N",
                             fishery %in% c("North Central T", "South Central T") ~ "Central T",
...
CN_catchdist_sum <- CN_catchdist_byarea %>%
  group_by(recY, prod_area) %>%
...
NCST_areaplot <- ggplot(subset(CN_catchdist_byarea, prod_area == "NCST"), aes(x = recY, y = percentage, fill = fishery)) +
  geom_area(alpha = 0.6, size = 0.4, colour = "black") +
  theme_bw() +
  theme(legend.position = "bottom",
        legend.key.size = unit(0.8, "line"),
...
CCST_areaplot <- ggplot(subset(CN_catchdist_byarea, prod_area == "CCST"), aes(x = recY, y = percentage, fill = fishery)) +
  geom_area(alpha = 0.6, size = 0.4, colour = "black") +
  theme_bw() +
  theme(legend.position = "bottom",
        legend.key.size = unit(0.8, "line"),
...
WCVI_areaplot <- ggplot(subset(CN_catchdist_byarea, prod_area == "WCVI"), aes(x = recY, y = percentage, fill = fishery)) +
  geom_area(alpha = 0.6, size = 0.4, colour = "black") +
  theme_bw() +
  theme(legend.position = "bottom",
        legend.key.size = unit(0.8, "line"),
...
ISC_areaplot <- ggplot(subset(CN_catchdist_byarea, prod_area == "ISC"), aes(x = recY, y = percentage, fill = fishery)) +
  geom_area(alpha = 0.6, size = 0.4, colour = "black") +
  theme_bw() +
  theme(legend.position = "bottom",
        legend.key.size = unit(0.8, "line"),
...
##---- CO ---- 
CO_catchdist_byarea <- CO_CatchbyFishery %>%
  mutate(fishery = case_when(fishery %in% c("Central Terminal N", "Central N", "PFMA8 Terminal N") ~ "Central N",
                             fishery %in% c("ADFG", "NMFS AK") ~ "AK",
                             fishery %in% c("North Central T", "South Central T") ~ "Central T",
                             fishery %in% c("Juan de Fuca S", "Alberni Canal S", "WCVI S") ~ "WCVI S",
...
CO_catchdist_sum <- CO_catchdist %>%
  group_by(recY, prod_area) %>%
  summarise(regional_catch = sum(totcatch))

#--- NCST ---
# Create a total catch plot for each region to go above the catch by fishery area plots
NCST_CWT_catch <- ggplot(subset(CO_catchdist_sum, prod_area == "NCST"), aes(x = recY, y = regional_catch)) +
  geom_bar(stat = "identity", position = "dodge", colour = "black", size = 0.5) +
  theme_bw() +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank()) +
...
NCST_areaplot <- ggplot(subset(CO_catchdist_byarea, prod_area == "NCST"), aes(x = recY, y = percentage, fill = fishery)) +
  geom_area(alpha = 0.6, size = 0.4, colour = "black") +
  theme_bw() +
  theme(legend.position = "bottom",
        legend.key.size = unit(0.8, "line"),
  ...
  scale_y_continuous(expand = c(0,0)) +
  labs(y = "Proportion of Annual CWT Catch")
#ggsave("../figs/distribution/NCST_CO_catchbyfishery_areaplot.png", width = 16, height = 12, units = "cm")

#Combine above total CWT catch plot with the catch by fishery area plot
ggarrange(NCST_CWT_catch, NCST_areaplot, nrow = 2, heights = c(5, 10), align = "v")
#ggsave("../figs/distribution/NCST_CO_catchdist_combo.png", width = 16, height = 16, units = "cm")



# --- CCST ---
CCST_CWT_catch<- ggplot(subset(CO_catchdist_sum, prod_area == "CCST"), aes(x = recY, y = regional_catch)) +
  geom_bar(stat = "identity", position = "dodge", colour = "black", size = 0.5) +
  theme_bw() +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank()) +
...
CCST_areaplot <- ggplot(subset(CO_catchdist_byarea, prod_area == "CCST"), aes(x = recY, y = percentage, fill = fishery)) +
  geom_area(alpha = 0.6, size = 0.4, colour = "black") +
  theme_bw() +
  theme(legend.position = "bottom",
        legend.key.size = unit(0.8, "line"),
  ...
  scale_y_continuous(expand = c(0,0)) +
  labs(y = "Proportion of Annual CWT Catch")
#ggsave("../figs/distribution/CCST_CO_catchbyfishery_areaplot.png", width = 16, height = 12, units = "cm")

#Combine above total CWT catch plot with the catch by fishery area plot
ggarrange(CCST_CWT_catch, CCST_areaplot, nrow = 2, heights = c(5, 10), align = "v")
#ggsave("../figs/distribution/CCST_CO_catchdist_combo.png", width = 16, height = 16, units = "cm")


# --- WCVI ---
WCVI_CWT_catch<- ggplot(subset(CO_catchdist_sum, prod_area == "WCVI"), aes(x = recY, y = regional_catch)) +
  geom_bar(stat = "identity", position = "dodge", colour = "black", size = 0.5) +
  theme_bw() +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank()) +
  labs(y = "Total Annual CWT Catch") +
  scale_x_continuous(expand = c(0,0), limits = c(1974, 2021)) +
  scale_y_continuous(expand = c(0,0), limits = c(0,75000), labels = comma)
WCVI_CWT_catch

WCVI_areaplot <- ggplot(subset(CO_catchdist_byarea, prod_area == "WCVI"), aes(x = recY, y = percentage, fill = fishery)) +
  geom_area(alpha = 0.6, size = 0.4, colour = "black") +
  theme_bw() +
  theme(legend.position = "bottom",
        legend.key.size = unit(0.8, "line"),
  ...
  scale_y_continuous(expand = c(0,0)) +
  labs(y = "Proportion of Annual CWT Catch")
#ggsave("../figs/distribution/WCVI_CO_catchbyfishery_areaplot.png", width = 16, height = 12, units = "cm")

#Combine above total CWT catch plot with the catch by fishery area plot
ggarrange(WCVI_CWT_catch, WCVI_areaplot, nrow = 2, heights = c(5, 10), align = "v")
#ggsave("../figs/distribution/WCVI_CO_catchdist_combo.png", width = 16, height = 16, units = "cm")



# --- ISC ---
ISC_CWT_catch<- ggplot(subset(CO_catchdist_sum, prod_area == "ISC"), aes(x = recY, y = regional_catch)) +
  geom_bar(stat = "identity", position = "dodge", colour = "black", size = 0.5) +
  theme_bw() +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank()) +
  labs(y = "Total Annual CWT Catch") +
  scale_x_continuous(expand = c(0,0), limits = c(1974, 2021)) +
  scale_y_continuous(expand = c(0,0), limits = c(0,650000), labels = comma)
ISC_CWT_catch

ISC_areaplot <- ggplot(subset(CO_catchdist_byarea, prod_area == "ISC"), aes(x = recY, y = percentage, fill = fishery)) +
  geom_area(alpha = 0.6, size = 0.4, colour = "black") +
  theme_bw() +
  theme(legend.position = "bottom",
        legend.key.size = unit(0.8, "line"),
  ...
  scale_y_continuous(expand = c(0,0)) +
  labs(y = "Proportion of Annual CWT Catch")
#ggsave("../figs/distribution/ISC_CO_catchbyfishery_areaplot.png", width = 16, height = 12, units = "cm")

#Combine above total CWT catch plot with the catch by fishery area plot
ggarrange(ISC_CWT_catch, ISC_areaplot, nrow = 2, heights = c(5, 10), align = "v")
#ggsave("../figs/distribution/ISC_CO_catchdist_combo.png", width = 16, height = 16, units = "cm")
...
# To calculate median and range catch efficiency for CN and CO in each region by objective since 2000 (recent efficiency)
Eff_CN_sum <- CN_obj_catch %>%
  filter(!prod_area %in% c("TRAN", "YUKN"), OEY > 1999) %>%
  mutate(prod_area = case_when(prod_area %in% c("CCST", "RIVR") ~ "CCST",
                               prod_area %in% c("NCST", "QCI", "SKNA", "NASS") ~ "NCST",
  ...
  group_by(prod_area, obj) %>%
  mutate(median_CPT = median(CPthousandR), min = min(CPthousandR), max = max(CPthousandR)) %>%
  select(1, 3, 10:12) %>%
  distinct() %>%
  arrange(prod_area, obj) %>%
  mutate(prod_area = factor(prod_area, levels = c("NCST", "CCST", "WCVI", "ISC")))
# For the data in Table 4 of the report
write_csv(Eff_CN_sum, "../processed/catch_eff_summary_CN.csv")
...
Eff_CO_sum <- CO_obj_catch %>%
  ...
  arrange(prod_area, obj) %>%
  mutate(prod_area = factor(prod_area, levels = c("NCST", "CCST", "WCVI", "ISC")))
# For the data for Table 4 in the harvest report
write_csv(Eff_CO_sum, "../processed/catch_eff_summary_CO.csv")
