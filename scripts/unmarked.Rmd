...
This code serves to calculate what proportion of hatchery releases are CWT'd, unmarked but associated with a CWT release group, or unmarked and unassociated with a CWT release group.
...
#ggsave("../figs/CWT_mark_unmark.png", width = 22, height = 12, units = "cm")
...
#write_csv(unmrk_rel_table, "../processed/percent_unmarked_releases.csv")
...
# plot annual releases and show ratio of CWT-associated and non-CWT-associated releases
...
props <- annual_regional_marked_rel %>%
  pivot_wider(names_from = "mark_type", values_from = "totrel") %>%
  mutate_at(vars('non-CWT', 'CWT'), ~replace_na(., 0)) %>%
  mutate(CWT_rate = CWT/(CWT+`non-CWT`)) %>%
  select(1:3, 6) 
